"""Build a SIGNED sample Meta webhook body for manual testing (api.rest).
    ...
    make webhook-sample kind=message from=919820011223 text="Hi"
...
def build(args: argparse.Namespace, phone_number_id: str) -> tuple[dict, str]:
    ...
    if args.kind == "message":
        value["contacts"] = [
            {"profile": {"name": "Test Customer"}, "wa_id": args.sender}
        ]
        value["messages"] = [
            {
                "from": args.sender,
...
def main() -> int:
    ...
    print(f"signature {compute_signature(body)}")  # noqa: T201
    return 0
